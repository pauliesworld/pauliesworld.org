...
# File: pworg_page_travels.pl
# 

sub pworg_page_travels()
{
    ($TRAVDIR) = @_;
    ...
    if ($param_location) {
        ...
        $locationptr=pworg_api_openfile("${TRAVDIR}${param_location}.txt");
...
HTML
        ...
        foreach $location (@traveltable) {
            chomp($location);
            $locationptr=pworg_api_openfile("${TRAVDIR}${location}");
            @locationinfo = @$locationptr;
